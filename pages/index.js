...
export default function Index() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(res => {
      if (res.data.session.expires_at > Date.now()) {
        setIsLoggedIn(true);
      }
    });
  }, []);
  ...
  return (
    <>
      <Drawer isLoggedIn={isLoggedIn} isOpen={isDrawerOpen} onDrawerClose={() => setIsDrawerOpen(false)} fixed />
      
      <DrawerNavbar onToggleDrawer={() => setIsDrawerOpen(!isDrawerOpen)} className="bg-gray-400 font-bold" />
      ...
      <section className="header relative pt-16 items-center flex h-screen max-h-860-px">
        ...
        <img
          className="absolute hidden top-0 b-auto right-0 pt-16 sm:w-6/12 z-0 -mt-48 sm:mt-0 w-10/12 max-h-860-px"
      ...
      <section className="block relative z-0 bg-blueGray-600">
        <div className="container mx-auto">
